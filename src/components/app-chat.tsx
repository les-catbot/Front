...
// --- Star Rating ---
function StarRating({ nota, onRate, disabled }: { nota: number | null; onRate: (n: number) => void; disabled: boolean; }) {
...
export function AppChat({ isLogged, userId, conversaId, onConversaIdChange, onConversaCriada }: AppChatProps) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/historico/conversas/${conversaId}`)
      ...
      .then((data) => {
        const msgs: Message[] = (data.mensagens ?? []).map((m: any, index: number) => ({
          id: index,
      ...
      .catch(() => {
        setMessages([{ id: 0, role: "assistant", content: "Não foi possível carregar o histórico desta conversa." }]);
      })
  ...
  const sendAvaliacao = async (messageId: number, nota: number, comentario: string, mensagemUUID: string) => {
    if (!userId) return;
    setSendingFeedback((prev) => ({ ...prev, [messageId]: true }));
    try {
      const body = { mensagem_id: mensagemUUID, usuario_id: userId, nota, comentario };
  ...
  // CORREÇÃO: Utilizar a rota do back-end em vez de window.print()
  const handleExportPDF = () => {
    if (!conversaId) return;
    // Abre a rota do FastAPI que devolve o PDF numa nova janela/separador
    window.open(`${API_URL}/exportacao/conversa/${conversaId}/pdf`, "_blank");
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: Date.now() + 1, role: "assistant", content: "Erro ao conectar com o servidor. Tente novamente." }]);
    } finally {
      setIsSending(false);
    }
  };
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-8 py-6">
        {isLoadingHistory ? (
          <div className="flex flex-1 items-center justify-center">
            <p className="text-sm text-neutral-400">A carregar conversa...</p>
          </div>
        ) : !hasMessages ? (
          <>
            ...
            <div className="flex flex-1 items-center justify-center px-4">
              <div className="relative w-full max-w-2xl">
                <input
                  ...
                  placeholder="Com o que precisas de ajuda?"
            ...
            <div className="mb-6 flex-1 space-y-4 overflow-y-auto pt-6">
              {messages.map((message) => (
                <div key={message.id} className={`flex flex-col ${message.role === "user" ? "items-end" : "items-start"}`}>
                  <div className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm shadow-sm ${message.role === "user" ? "bg-[#4a90c2] text-white" : "bg-white text-neutral-800"}`}>
                    {message.content}
                  </div>
